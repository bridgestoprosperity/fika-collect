...
export async function PUT(request) {
    try {
        ...
        if (survey.id !== id) {
            return new Response(JSON.stringify({ error: 'Survey ID mismatch between URL and body' }), {
                status: 400,
                headers: {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin': '*',
        ...
        // Update the manifest for backward compatibility with older clients
        await updateManifest(s3);
        return new Response(JSON.stringify({ success: true, survey_id: id }), {
            status: 200,
            headers: {
                'Content-Type': 'application/json',
                'Access-Control-Allow-Origin': '*',
...
export async function DELETE(request) {
    try {
        ...
        return new Response(JSON.stringify({ error: 'Failed to delete survey' }), {
            status: 500,
            headers: {
                'Content-Type': 'application/json',
                'Access-Control-Allow-Origin': '*',
