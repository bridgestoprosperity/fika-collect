 ...
 */
export async function updateManifest(s3: S3Client): Promise<void> {
  try {
    ...
    const manifestSurveys: ManifestSurvey[] = [];
    ...
    for (const file of surveyFiles) {
      ...
      try {
        ...
        const survey = JSON.parse(bodyString) as Survey;
